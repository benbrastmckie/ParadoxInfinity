...
\begin{enumerate}
  ...
  \item[\it Injection Principle:] $\abs{A} \leq \abs{B}$ \textit{iff} $A \simeq C$ for some $C\subseteq B$. 
    ...
    \end{itemize}
  \item[\it Same Number:] $\abs{A} = \abs{B}$ \textit{iff} $\abs{A} \leq \abs{B}$ and $\abs{B} \leq \abs{A}$.
  \item[\it Strict Order:] $\abs{A} < \abs{B}$ \textit{iff} $\abs{A} \leq \abs{B}$ and $\abs{B} \nleq \abs{A}$.
\end{itemize}
  ...
  \item[\it Infinite:] A set $A$ is \textit{infinite iff} $\abs{\N} \leq \abs{A}$.
    \begin{itemize}[leftmargin=-.2in]
      ...
      \item The positive rational numbers $\Q^+$ are countably infinite since: 
        \begin{itemize}
          \item There is an injection from $\Q^+$ to $\set{\tuple{n,m}\in\N^2: m \neq 0 \text{ and } n\bot m}$ 
          \item Countable since $f(\tuple{n,m})=2^n \cdot 3^m$ is an injection from $\Q$ to $\N$.
...
\begin{enumerate}
  ...
  \item[\it Theorem:] $\abs{A} < \abs{\wp(A)}$ for any set $A$. 
